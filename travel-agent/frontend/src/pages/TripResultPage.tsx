...
import { InteractiveRouteSection } from "../components/InteractiveRouteSection";
import { ItineraryHighlightsRail } from "../components/ItineraryHighlightsRail";
...
import { parseScheduleSlotsByDay } from "../lib/parseScheduleSlots";
...
export function TripResultPage() {
  ...
  const scheduleByDay = useMemo(
    () => parseScheduleSlotsByDay(data?.itinerary ?? ""),
    [data?.itinerary],
  );
  ...
  async function copyItinerary() {
    const t = saved.response.itinerary ?? "";
    try {
                ...
                <div className="space-y-12">
                  <div className="flex flex-wrap items-center gap-3">
                    <button
                      ...
                      onClick={() => setMapOpen(true)}
                      disabled={!hasMapData}
                      className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-primary to-secondary px-5 py-2.5 text-sm font-bold text-white shadow-md disabled:opacity-40"
                    >
                    ...
                    <button
                      ...
                      className="rounded-full border border-slate-200 px-5 py-2.5 text-sm font-semibold text-muted hover:border-primary/40 hover:text-primary"
                    >
                      {showRawMarkdown ? "Hide raw text" : "Show raw markdown"}
                    </button>
                    <button
                      type="button"
                      onClick={copyItinerary}
                      className="rounded-full border border-slate-200 px-5 py-2.5 text-sm font-semibold text-muted hover:text-primary"
                    >
                      Copy
                    </button>
                    <button
                  ...
                  <ItineraryHighlightsRail days={scheduleByDay} />
                  ...
                  {showRawMarkdown && (
                    <motion.div
                      ...
                      className="overflow-hidden rounded-2xl border border-dashed border-slate-200 bg-surface-low/30 p-6"
                    >
                      <p className="mb-3 text-xs font-bold uppercase tracking-widest text-muted">
                        Source markdown
                      </p>
                      <div className="max-h-96 overflow-y-auto text-xs leading-relaxed text-muted">
                        <ReactMarkdown>
                          {data.itinerary ?? "_Empty._"}
                        </ReactMarkdown>
                      </div>
                    </motion.div>
                  )}
